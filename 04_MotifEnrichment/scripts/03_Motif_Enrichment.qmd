...
perform_motif_analysis <- function(fg.seqs, bg.seqs) {
    mot <- motifx(fg.seqs, bg.seqs, central.res = 'Y', min.seqs = 15, pval.cutoff = 0.05)
...
unique_foreground_motifs_15aalong <- unique_foreground_motifs[nchar(unique_foreground_motifs)==15]
nchar(unique_foreground_motifs)

unique_background_motifs_15aalong <- unique_background_motifs[nchar(unique_background_motifs)==15]


unique_DECOYforeground_motifs_15aalong <- unique_DECOYforeground_motifs[nchar(unique_DECOYforeground_motifs)==15]



# Check the lengths of each k-mer in fg.seqs
kmer_lengths <- sapply(unique_foreground_motifs_15aalong, nchar)

# Report if there are k-mers of differing lengths
if(length(unique(kmer_lengths)) > 1) {
  cat("Foreground k-mers are of varying lengths:\n")
  print(table(kmer_lengths))
} else {
  cat("All foreground k-mers are of the same length.\n")
}


fg_plots <- perform_motif_analysis(unique_foreground_motifs_15aalong, unique_background_motifs_15aalong)

# For unique_DECOYforeground_motifs
decoy_fg_plots <- perform_motif_analysis(unique_DECOYforeground_motifs_15aalong, unique_background_motifs_15aalong)
