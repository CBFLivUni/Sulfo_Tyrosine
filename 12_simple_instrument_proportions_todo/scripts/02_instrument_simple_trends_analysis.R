...
data_dir <- "C:/Users/jtzve/Desktop/Sufo_Tyrosine/07_BOI_peptidoforms_Colourcoded_histograms/out/counts_all//"

#TODO: re-run with new peptidoforms added; split groups into all dataset, all BOI, 
# convincing histograms BOI  - further split into with and without biological context?
# known sY and false positives or undetermined only
...
instruments_contingency_data <- instruments_contingency_data %>%
  mutate(PSM_totals_BOI = rowSums(dplyr::select(., -c(instrument, PSM_totals_BOI, PSM_proportions_BOI,PSM_percentages_BOI)), na.rm = TRUE))
...
# NB2: Calumenin added as sY site is confirmed in the literature; used annotation v2 as of 20/05/24
...
head(baloon_plot_percentages_data_to_plot)
# get rid of the < 5% data

subset <- baloon_plot_percentages_data_to_plot %>%
  filter(percentage > 1)

baloon_plot <- ggplot(subset, aes(x = instrument, y = category, size = percentage, color = category)) +
...
ggsave(plot = baloon_plot, 
       filename = "../out/PSMs_Percent_contribution_by_category_baloonplot_morethan1_v2.png", 
       dpi = 600, width = 10, height = 8)

# generate bar charts for all data

plot_and_save(data = STB_A3_log2, protein_meta = protein_meta, 
              output_dir = "../out/all_tissues_together/", data_name = "ALL")
