...
# manually assmebled the results and fixed incorrect data formats to get this .csv prepared for plotting.
data_to_plot <- read.csv("../out/ORA/ORA_master_results_toplot.csv")[,1:5]

data_to_plot$Bin_Tag <- gsub("DECOY", "D", data_to_plot$Bin_Tag)

# convert Bin_Tag to a factor and set levels in the desired order
data_to_plot$Bin_Tag <- factor(data_to_plot$Bin_Tag, levels = c("BOI2", "BOI1", "D1", "D2", "D3"))

# get rid of " at start of cells in columns Max_Genes and Detected_genes 
data_to_plot$Max_Genes <- gsub('\"', '', data_to_plot$Max_Genes)
data_to_plot$Detected_Genes <- gsub('\"', '', data_to_plot$Detected_Genes)
...
custom_colors <- c("BOI2" = "#E69F00", "BOI1" = "#E69F00", "D1" = "#56B4E9", "D2" = "#56B4E9", "D3" ="#56B4E9")
